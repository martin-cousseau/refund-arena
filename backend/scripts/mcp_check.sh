...
# Refund Arena: compose.yaml lives at the repo root (parent of backend/).
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [ -f "${SCRIPT_DIR}/../../compose.yaml" ]; then
  cd "${SCRIPT_DIR}/../.."
fi
SERVICE="${COMPOSE_SERVICE:-backend}"
...
echo ""
echo -e "${DIM}> http://localhost:8000/mcp  (client runs inside ${SERVICE})${NC}"
echo ""

if docker compose exec -T "$SERVICE" python -u - "$QUESTION" "$EXPECTED_TOOLS" <<'PY'
import asyncio
import sys
import time
...
then
    ...
    echo ""
    echo -e "${RED}${BOLD}Failed.${NC} ${DIM}Check the container: docker compose logs backend${NC}"
    echo ""
    exit 1
fi
