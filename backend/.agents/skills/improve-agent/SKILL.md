...
- `curl -sSf http://localhost:8000/health` returns 200; the container is bound to this checkout (`docker inspect refund-arena-backend --format '{{range .Mounts}}{{.Source}}{{"\n"}}{{end}}' | grep -F "$(pwd)"` prints a line).
...
docker logs refund-arena-backend --since 30s 2>&1 | grep -E "Running: \w+\(" | head -40
```
...
docker exec refund-arena-backend grep -c "<unique substring>" /app/agents/<slug>.py   # 0 = edit didn't reach the container
```
