...
This checkout is the Refund Arena monorepo. Run Compose from the **repository root** (the directory that contains `compose.yaml` and `backend/`). The API service is named `backend`. `.env` lives at the repo root (`cp .env.example .env`).
...
From the repo root, `cp .env.example .env`, then set `OPENAI_API_KEY`:
...
From the repo root, `docker compose up -d --build`, then poll http://localhost:8000/docs until 200 (first build takes minutes). If it never comes up, read `docker compose logs backend`.

## 4. Prove it

`./backend/scripts/mcp_check.sh` (from the repo root) prints "MCP OK" and a real agent answer. Quote the answer — it's their Platform Manager — and say the MCP server is live.
