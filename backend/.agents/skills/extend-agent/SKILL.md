...
- The container is bound to *this* checkout: `docker inspect refund-arena-backend --format '{{range .Mounts}}{{.Source}}{{"\n"}}{{end}}' | grep -F "$(pwd)"` prints a line. Empty → `cd` to the bound repo or restart compose from here.
...
```bash
docker exec refund-arena-backend python -c "import inspect, agno.agent; print(inspect.signature(agno.agent.Agent.__init__))"
docker exec refund-arena-backend python -c 'from agno.tools.exa import ExaTools'   # verify EVERY new import in the container
```
...
- **Grow the registry** — the bucket you declare into decides how a built component references it (`tool_names`, `model_id`, `knowledge_name`, `learning_name`, `function_name`). Tool names are global to an agent, so a toolkit exposing a name another declared toolkit already has is silently dropped — check first, and give any new toolkit `add_instructions=True` (a built component's instructions are model-written; this is your only usage channel):
  ...
  ```bash
  docker exec refund-arena-backend python -c "from app.registry import registry; print([t.name for t in registry.tools if 'read_file' in t.functions])"
  ```
...
```bash
docker compose restart backend                                   # no new deps
...
docker exec refund-arena-backend grep -c "<unique substring from your edit>" /app/agents/<slug>.py   # 0 = edit didn't reach the container
```
...
docker logs refund-arena-backend --since 30s 2>&1 | grep -E "Running: \w+\(" | head -40   # which tools fired
```
