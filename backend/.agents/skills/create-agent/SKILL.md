...
```bash
docker exec refund-arena-backend python -c 'from agno.tools.exa import ExaTools'
```
...
```bash
docker exec refund-arena-backend python -c "import asyncio; from app.tools import get_knowledge_management_tools; \
...
- **404** — not registered, not restarted, or the container is bound to another checkout: `docker inspect refund-arena-backend --format '{{range .Mounts}}{{.Source}}{{"\n"}}{{end}}'`.
- **5xx** — `docker logs refund-arena-backend --tail 50`: usually an import error, a missing env var, or a typo in `tools=`.
