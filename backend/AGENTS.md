...
| [`workflows/deployment_check.py`](workflows/deployment_check.py) | Reference workflow — a deterministic `Step` that checks DB, auth, SuperGrok sign-in, scheduler URL, MCP reachability, Slack config, component imports, registry names, schedule state, and poller liveness; imported into `app/main.py` and passed to `AgentOS(workflows=[...])`. |
...
```bash
cp example.env .env
# Edit .env and set XAI_TOKEN_ENCRYPTION_KEY, then:
#   docker compose exec backend python -m app.xai_login

docker compose up -d --build
```

The first boot opens MCP connections to `https://docs.agno.com/mcp` and — unless `PARALLEL_API_KEY` is set — `https://search.parallel.ai/mcp`. Those handshakes are fail-soft (`OptionalMCPTools` in [`app/tools.py`](app/tools.py)): a 429 or unreachable host logs a warning and the API still starts — Platform Builder / Agno lose that toolkit until the next successful connect (`refresh_connection=True`). Builder, the Agno team, and the registry share one instance per server so a reload does not open duplicates. Behind a proxy or firewall, allow both, or set `PARALLEL_API_KEY` to drop the second.
...
|---|---|---|---|
| `XAI_TOKEN_ENCRYPTION_KEY` | yes | — | Encrypts the SuperGrok OAuth session in Postgres. Generate with `python -c "from agno.utils.encryption import generate_encryption_key; print(generate_encryption_key())"`. Rotating it makes the stored session unreadable until you sign in again. |
| `XAI_API_KEY` | no | — | Metered xAI fallback when no SuperGrok session exists (or OAuth inference is refused). Deployment check warns on this path. |
| `OPENAI_API_KEY` | no | — | Optional. Studio image/TTS only. Models and embeddings do not use it. |
